...
def create_model(data_format):
  ...
  return tf.keras.Sequential(layers=[l.Reshape(target_shape, input_shape=[784]),
          l.Conv2D(
              32,
              5,
              padding='same',
...
class MNISTModel(tf.keras.Model):
  def __init__(self, data_format):
    ...
    if data_format == 'channels_first':
        self.target_shape = [1, 28, 28]
    else:
        assert data_format == 'channels_last'
        self.target_shape = [28, 28, 1]

    self.reshape = tf.keras.layers.Reshape(self.target_shape, input_shape=[784])
    ...
    self.max_pool = tf.keras.layers.MaxPooling2D((2, 2), (2, 2), padding='same', data_format=data_format)
